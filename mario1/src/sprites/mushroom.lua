local sprite = BaseSprite:extend({ type = 'mushroom', collision = 'cross' })
...
function sprite:constructor (x, y, ref)
  ...
  self.behavior = Behavior {
    default = {
      { duration = 1, quad = Quads.sprites[11][1] },
    },
  }
    ...
    if self.dx ~= 0 then
      ...
      for _, col in pairs(cols) do
        ...
        if col.other.type == 'mario' then
          Sounds.powerup:play()
          col.other:setControllable(false)
          col.other.behavior:setState('grow')
          self:destroy()
        end
      end
    end
